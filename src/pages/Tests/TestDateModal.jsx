...
export default function TestDateModal({ isOpen, onClose, onSubmit, test, isLoading = false }) {
  const isReschedule = test?.testStatus === 'Failed';
  ...
  useEffect(() => {
    if (isOpen) {
      setTestDate(
        test?.testDate ? new Date(test.testDate).toISOString().split('T')[0] : getTodayLocal()
      );
      setError('');
    }
  }, [isOpen, test]);
      ...
      <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-gray-200 p-6">
          <h2 className="text-xl font-bold text-[var(--color-forest-deep)]">
            {isReschedule ? 'Reschedule Test' : 'Set Test Date'}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-6">
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--color-forest-deep)]">
              {isReschedule ? 'Next Test Date' : 'Test Date'} <span className="text-red-500">*</span>
            </label>
            <input
              type="date"
              value={testDate}
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {isLoading ? 'Saving...' : isReschedule ? 'Reschedule' : 'Save Date'}
            </button>
          </div>
        </form>
      </div>
